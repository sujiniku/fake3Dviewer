// fake3Dviewer.cpp : アプリケーションのエントリ ポイントを定義します。
//

#include "framework.h"
#include "fake3Dviewer.h"
	...
	_In_ int       nCmdShow)
{
	...
	LoadStringW(hInstance, IDC_FAKE3DVIEWER, szWindowClass, MAX_LOADSTRING);
	...
	HACCEL hAccelTable = LoadAccelerators(hInstance, MAKEINTRESOURCE(IDC_FAKE3DVIEWER));
...
ATOM MyRegisterClass(HINSTANCE hInstance)
{
	...
	wcex.hIcon = LoadIcon(hInstance, MAKEINTRESOURCE(IDI_FAKE3DVIEWER));
	wcex.hCursor = LoadCursor(nullptr, IDC_ARROW);
	wcex.hbrBackground = (HBRUSH)(COLOR_WINDOW + 1);
	wcex.lpszMenuName = MAKEINTRESOURCEW(IDC_FAKE3DVIEWER);
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_PAINT:
	{
		...
		double costheta = naiseki1 / zettai1;
		...
		int bufseisuu = (int)100 * costheta;
		...
		float theta = (float)acos(naiseki1 / zettai1);


		float bairitu2 = theta / 0.3;

		bufseisuu = (int)100 * theta;
